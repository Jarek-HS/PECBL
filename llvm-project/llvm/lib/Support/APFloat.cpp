  ...
  if(S.sizeInBits != 128 && S.sizeInBits != 16){
    if (llvm::use_posit && (posit.is_change==1||posit.is_change==2) ) {
      ...
      if (!posit.posit_tag) {
        ...
        uint64_t fraction = value;
        fraction = fraction << 2;
        int count = 1;
        int r;
        uint64_t sign = this->sign;
        int scale = this->exponent;
        int kA = scale / 4;
        int expA = scale % 4;
        if (scale < 0) {
            expA = 4 + expA;
        ...
        uint64_t k = 1;
        uint64_t tmp_k;
        uint64_t fraction_loss =0 , tmp_exponent = 0;
        bool rounding = false;
        tmp_exponent = expA;
        int regime_shift;
        ...
        tmp_exponent = (tmp_exponent << (regime_shift - 3));
        number = (number | tmp_exponent);
        fraction_loss = (fraction << (64-((64 - regime_shift) + 3)));
        if (fraction_loss >= 0x1000000000000000) rounding = true;
