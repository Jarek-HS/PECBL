//===-- AFloat.cpp - Implement APFloat class -----------------------------===//
//
...
static constexpr fltSemantics semPositsingle = {120, -120, 32, 32};
static constexpr fltSemantics semPositdouble = {120, -120, 64, 64};
...
void IEEEFloat::assign(const IEEEFloat &rhs) {
  ...
  posit.is_change = rhs.posit.is_change;
  posit.posit_tag = rhs.posit.posit_tag;
  posit.exponent = rhs.posit.exponent;
  posit.fraction = rhs.posit.fraction;
  posit.posit_part = rhs.posit.posit_part;
  posit.sign = rhs.posit.sign;
  posit.regime = rhs.posit.regime;
  APInt::tcAssignPoist(posit_significand(), rhs.posit_significand(),
                       posit_tag(), rhs.posit_tag());
...
bool IEEEFloat::bitwiseIsEqual(const IEEEFloat &rhs) const {
  ...
  if (isFiniteNonZero() && exponent != rhs.exponent)
    return false;
  if(llvm::use_posit){
    if(posit.is_change==1 && significand.part==0)return true;
    else if(posit.is_change==1) return false ;
  }
...
// posit
const IEEEFloat::integerPart *IEEEFloat::posit_significand() const {
  return const_cast<IEEEFloat *>(this)->posit_significand();
}
IEEEFloat::integerPart *IEEEFloat::posit_significand() {
  return &posit.posit_part;
}
bool *IEEEFloat::posit_tag() { return &posit.posit_tag; }
int *IEEEFloat::posit_ischange() { return &posit.is_change; }
const bool *IEEEFloat::posit_tag() const { return &posit.posit_tag; }
IEEEFloat::integerPart *IEEEFloat::posit_sign() { return &posit.sign; }
IEEEFloat::integerPart *IEEEFloat::posit_exponent() { return &posit.exponent; }
IEEEFloat::integerPart *IEEEFloat::posit_regime() { return &posit.regime; }
IEEEFloat::integerPart *IEEEFloat::posit_fraction() { return &posit.fraction; }
...
IEEEFloat::opStatus IEEEFloat::add(const IEEEFloat &rhs,
  ...
  if(!llvm::use_posit)
    ...
    sw::universal::posit<64,2> a ;
    sw::universal::posit<64,2> b ;
    if(sign==1&& ((posit.posit_part&0x8000000000000000) == 0)) a.setbits(~posit.posit_part);
    else a.setbits(posit.posit_part);
    if(rhs.sign==1&& ((rhs.posit.posit_part&0x8000000000000000) == 0)) b.setbits(~rhs.posit.posit_part);
    else  b.setbits(rhs.posit.posit_part);
    a=a+b;
    sign = a.isneg();
    posit.posit_part = a.bits();
    posit.posit_tag = false;
    posit.is_change = 1;
    return opInexact;
}
...
IEEEFloat::opStatus IEEEFloat::fusedMultiplyAdd(const IEEEFloat &multiplicand,
  ...
  opStatus fs;
  if(llvm::use_posit){
    multiply(multiplicand,rounding_mode);
    add(addend,rounding_mode);
    return opInexact;
  }
...
IEEEFloat::opStatus IEEEFloat::roundToIntegral(roundingMode rounding_mode) {
  ...
  if(llvm::use_posit){
    ...
    double tmp = (double)a;
    int tmp1 = (int)tmp;
    sw::universal::posit<64,2> b(tmp1);
    posit.posit_part = b.bits();
    return opInexact;
  }
  if (isInfinity())
...
IEEEFloat::roundSignificandWithExponent(const integerPart *decSigParts,
  ...
  for (;; parts *= 2) {
    ...
    if (HUdistance >= HUerr) {
      APInt::tcPosit(posit_significand(), decSig.significandParts());
      APInt::tcExtract(
...
APInt IEEEFloat::convertIEEEFloatToAPInt() const {
  ...
  uint64_t numbers[2];
  assert(semantics == &S);
  if(S.sizeInBits != 128 && S.sizeInBits != 16){
    llvm::errs() << "posit part: " << posit.posit_part << "\n";
    if (llvm::use_posit && (posit.is_change==1||posit.is_change==2) ) {
      number = posit.posit_part;
      if(S.sizeInBits == 32 && (number>>32)!=0) {
        if(sign==1 && ((number&0x8000000000000000) == 0)) number =~number;
          tmp = number>>32;
          return APInt(S.sizeInBits, tmp);
        }else if(S.sizeInBits == 32 && (number>>32)==0){
          if(sign==1 && ((number&0x80000000) == 0)) number =~number;
          tmp = number;
          return APInt(S.sizeInBits, tmp);
        }else {
          if(sign==1 && ((number&0x8000000000000000) == 0)) number =~number;
          return APInt(S.sizeInBits,number);
        }
      }
    else if(llvm::use_posit && posit.is_change!=1 && posit.is_change!=2){
      if (!posit.posit_tag) {
        if(posit.posit_part==0) return APInt(S.sizeInBits, 0);
        uint64_t value = this->posit.posit_part;
        uint64_t mask = 0x8000000000000000;
        uint64_t fraction_loss =0;
        uint64_t regime_shift = 0;
        uint64_t posit.posit_fraction = value;
        int scale = exponent;
        int kA = scale / 4;
        int expA = scale % 4;
        if (scale < 0) {
        ...
        regime = (regime >> 1);
        number = (number | regime);
        exponent = (exponent << (regime_shift - 3));
        number = (number | exponent);
        fraction_loss = (posit.posit_fraction << (64-((64 - regime_shift) + 3)));
        if (fraction_loss >= 0x0100000000000000) rounding = true;
        posit.posit_fraction = (posit.posit_fraction >> ((64 - regime_shift) + 3));
        posit = (number | posit.posit_fraction);
        if (rounding) posit++;
        if (sign == mask) number = ~number;
        llvm::errs() << "posit: " << number << "\n";
          if(S.sizeInBits == 32) {
            tmp = number>>32;
            return APInt(S.sizeInBits, tmp);
          }
        ...
        number = 0;
        number |= posit.regime;
        number |= posit.exponent;
        number |= posit.fraction;
        if (sign == 1) {
          number = ~number;
        }
        if(S.sizeInBits == 32) {
    ...
    llvm::errs() << "IEEE=" << words[0] << "\n";
    llvm::errs() << "exponent: " << myexponent << "\n";
    if constexpr (last_word == 0) {
      if(llvm::use_posit){
        if(S.sizeInBits ==64){
        double d ;
        std::memcpy(&d,&words[0],sizeof(d));
        sw::universal::posit<64,2> p_d = (long double)d;
        uint64_t input = p_d.bits();
        return APInt(S.sizeInBits, input);
      }else{
        float d ;
        std::memcpy(&d,&words[0],sizeof(d));
        sw::universal::posit<32,2> p_d = (long double)d;
        uint32_t input = p_d.bits();
        return APInt(S.sizeInBits, input);
      }
  ...
  if(llvm::use_posit){
    long double ld ;
    std::memcpy(&ld,&words[0],sizeof(ld));
    sw::universal::posit<128,2> p_d = ld;
    uint64_t input[2];
    std::memcpy(input, &ld, sizeof(ld));
    return APInt(S.sizeInBits, input);
  }
...
void IEEEFloat::initFromIEEEAPInt(const APInt &api) {
  ...
  if (llvm::use_posit && S.sizeInBits!=128 && S.sizeInBits!=16) {
    this->posit.posit_part = *api.getRawData();
    this->posit.posit_tag = true;
    category = fcNormal;
    posit.is_change =1 ;
    posit.sign = this->posit.posit_part & 0x8000000000000000;
    if(*api.getRawData()==0){
      category = fcNormal;
      posit.is_change =2;
    }
  ...
  if constexpr (S.nonFiniteBehavior == fltNonfiniteBehavior::IEEE754) {
    if (!llvm::use_posit &&(myexponent - bias == ::exponentInf(S) && all_zero_significand)) {
      makeInf(sign);
      return;
    }
  }
...
void IEEEFloat::toString(SmallVectorImpl<char> &Str, unsigned FormatPrecision,
  ...
  if(llvm::use_posit){
    sw::universal::posit<64,2> p = posit.posit_part;
    std::string s = sw::universal::to_string(p);
    Str.append(s.data(), s.data() + s.size());
  }else{
...
void IEEEFloat::makeZero(bool Negative) {
  ...
  APInt::tcSet(significandParts(), 0, partCount());
  APInt::tcinitPosit(posit_significand(), posit_exponent(), posit_regime(),
                     posit_fraction(), posit_sign(), posit_tag(),posit_ischange());
}
