...
public:
  ...
  Value *EmitMul(const BinOpInfo &Ops) {
    ...
    if (CGF.getLangOpts().Fposit){
      if(Ops.LHS->getType()->isDoubleTy()) {
        llvm::FunctionCallee PositMul =
          CGF.CGM.getModule().getOrInsertFunction("posit64_2_mul",
                                                  Ops.LHS->getType(),
                                                  Ops.LHS->getType(),
                                                  Ops.RHS->getType());
      return CGF.Builder.CreateCall(PositMul, {Ops.LHS, Ops.RHS}, "posit64_mul_call");
      }else if(Ops.LHS->getType()->isFloatTy()) {
        llvm::FunctionCallee PositMul =
          CGF.CGM.getModule().getOrInsertFunction("posit32_2_mul",
                                                  Ops.LHS->getType(),
                                                  Ops.LHS->getType(),
                                                  Ops.RHS->getType());
        return CGF.Builder.CreateCall(PositMul, {Ops.LHS, Ops.RHS}, "posit32_mul_call");
      }
    }
...
Value *ScalarExprEmitter::VisitCastExpr(CastExpr *CE) {
  ...
  case CK_IntegralToFloating: {
    ...
    if(CGF.getLangOpts().Fposit ) {
      Value *Src = Visit(CE->getSubExpr());
      QualType srcTy = CE->getSubExpr()->getType();
      QualType dstTy = CE->getType();
      // int -> float (si_to_posit32)
      if(srcTy->isSpecificBuiltinType(BuiltinType::UInt)&& dstTy->isSpecificBuiltinType(BuiltinType::Float)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("uint32_to_posit32");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFloatTy(CGF.getLLVMContext()),
              { llvm::Type::getInt32Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "uint32_to_posit32", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "uint32_to_posit32");
      }
      if(srcTy->isSpecificBuiltinType(BuiltinType::UInt) && dstTy->isSpecificBuiltinType(BuiltinType::Double)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("uint32_to_posit64");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getDoubleTy(CGF.getLLVMContext()),
              { llvm::Type::getInt32Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "uint32_to_posit64", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "uint32_to_posit64");
      }
      if(srcTy->isSpecificBuiltinType(BuiltinType::ULong)&& dstTy->isSpecificBuiltinType(BuiltinType::Float)){
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("uint64_to_posit32");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFloatTy(CGF.getLLVMContext()),
              { llvm::Type::getInt64Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "uint64_to_posit32", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "uint64_to_posit32");
      }
      if(srcTy->isSpecificBuiltinType(BuiltinType::ULong) && dstTy->isSpecificBuiltinType(BuiltinType::Double)){
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("uint64_to_posit64");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getDoubleTy(CGF.getLLVMContext()),
              { llvm::Type::getInt64Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "uint64_to_posit64", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "uint64_to_posit64");
      }
      if (srcTy->isSpecificBuiltinType(BuiltinType::Int) && dstTy->isSpecificBuiltinType(BuiltinType::Float)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("int32_to_posit32");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFloatTy(CGF.getLLVMContext()),
              { llvm::Type::getInt32Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "int32_to_posit32", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "int32_to_posit32");
      }

      // int -> double (si_to_posit64)
      if (srcTy->isSpecificBuiltinType(BuiltinType::Int) && dstTy->isSpecificBuiltinType(BuiltinType::Double)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("int32_to_posit64");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getDoubleTy(CGF.getLLVMContext()),
              { llvm::Type::getInt32Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "int32_to_posit64", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "int32_to_posit64");
      }
      if (srcTy->isSpecificBuiltinType(BuiltinType::Long) && dstTy->isSpecificBuiltinType(BuiltinType::Float)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("int64_to_posit32");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFloatTy(CGF.getLLVMContext()),
              { llvm::Type::getInt32Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "int64_to_posit32", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "int64_to_posit64");
      }
      if (srcTy->isSpecificBuiltinType(BuiltinType::Long) && dstTy->isSpecificBuiltinType(BuiltinType::Double)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("int64_to_posit64");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getDoubleTy(CGF.getLLVMContext()),
              { llvm::Type::getInt64Ty(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "int64_to_posit64", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "int64_to_posit64");
      }
  }
  ...
  if (CGF.getLangOpts().Fposit) {
    Value *Src = Visit(CE->getSubExpr());
    // int -> float
    if (srcTy->isSpecificBuiltinType(BuiltinType::Float)&&dstTy->isSpecificBuiltinType(BuiltinType::Int)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit32_to_int32");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt32Ty(CGF.getLLVMContext()),
            { llvm::Type::getFloatTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit32_to_int32", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit32_to_int32");
    }

    // int -> double
    if (srcTy->isSpecificBuiltinType(BuiltinType::Double)&&dstTy->isSpecificBuiltinType(BuiltinType::Int)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit64_to_int32");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt32Ty(CGF.getLLVMContext()),
            { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit64_to_int32", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit64_to_int32");
    }
    if (srcTy->isSpecificBuiltinType(BuiltinType::Float)&&dstTy->isSpecificBuiltinType(BuiltinType::Long)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit32_to_int64");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt64Ty(CGF.getLLVMContext()),
            { llvm::Type::getFloatTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit32_to_int64", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit32_to_int64");
    }

    // int -> double
    if (srcTy->isSpecificBuiltinType(BuiltinType::Double)&&dstTy->isSpecificBuiltinType(BuiltinType::Long)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit64_to_int64");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt64Ty(CGF.getLLVMContext()),
            { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit64_to_int64", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit64_to_int64");
    }
    if (srcTy->isSpecificBuiltinType(BuiltinType::Float)&&dstTy->isSpecificBuiltinType(BuiltinType::UInt)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit32_to_uint32");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt32Ty(CGF.getLLVMContext()),
            { llvm::Type::getFloatTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit32_to_uint32", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit32_to_uint32");
    }

    // int -> double
    if (srcTy->isSpecificBuiltinType(BuiltinType::Double)&&dstTy->isSpecificBuiltinType(BuiltinType::UInt)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit64_to_uint32");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt32Ty(CGF.getLLVMContext()),
            { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit64_to_uint32", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit64_to_uint32");
    }
    if (srcTy->isSpecificBuiltinType(BuiltinType::Float)&&dstTy->isSpecificBuiltinType(BuiltinType::ULong)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit32_to_uint64");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt64Ty(CGF.getLLVMContext()),
            { llvm::Type::getFloatTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit32_to_uint64", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit32_to_uint64");
    }

    // int -> double
    if (srcTy->isSpecificBuiltinType(BuiltinType::Double)&&dstTy->isSpecificBuiltinType(BuiltinType::ULong)) {
      llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit64_to_uint64");
      if (!convFunc) {
        llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt64Ty(CGF.getLLVMContext()),
            { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
        convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                          "posit64_to_uint64", &CGF.CGM.getModule());
      }
      return Builder.CreateCall(convFunc, Src, "posit64_to_uint64");
    }
  }
    if (E->getType()->isVectorType() && DestTy->isVectorType()) {
      // TODO: Support constrained FP intrinsics.
  ...
  case CK_FloatingCast: {
    ...
    if (CGF.getLangOpts().Fposit) {
      ...
      // float -> double (posit32 -> posit64)
      if (srcTy->isSpecificBuiltinType(BuiltinType::Float) &&
          dstTy->isSpecificBuiltinType(BuiltinType::Double)) {
  
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit32_to_posit64");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getDoubleTy(CGF.getLLVMContext()),
              { llvm::Type::getFloatTy(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "posit32_to_posit64", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "posit32_to_posit64");
      }
  
      // double -> float (posit64 -> posit32)
      if (srcTy->isSpecificBuiltinType(BuiltinType::Double) &&
          dstTy->isSpecificBuiltinType(BuiltinType::Float)) {
  
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit64_to_posit32");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFloatTy(CGF.getLLVMContext()),
              { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "posit64_to_posit32", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "posit64_to_posit32");
      }
      // posit64 -> long double
      if (srcTy->isSpecificBuiltinType(BuiltinType::Double) &&
      dstTy->isSpecificBuiltinType(BuiltinType::LongDouble)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit64_to_LD");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFP128Ty(CGF.getLLVMContext()),
              { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "posit64_to_LD", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "posit64_to_ld");
      }
      // long double -> posit64
      if (srcTy->isSpecificBuiltinType(BuiltinType::LongDouble) &&
      dstTy->isSpecificBuiltinType(BuiltinType::Double)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("LD_to_posit64");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFP128Ty(CGF.getLLVMContext()),
              { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "LD_to_posit64", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "ld_to_posit64");
      }
      // posit32 -> long double
      if (srcTy->isSpecificBuiltinType(BuiltinType::Float) &&
      dstTy->isSpecificBuiltinType(BuiltinType::LongDouble)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("posit32_to_LD");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFP128Ty(CGF.getLLVMContext()),
              { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "posit32_to_LD", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "posit32_to_ld");
      }
      // long double -> posit32
      if (srcTy->isSpecificBuiltinType(BuiltinType::LongDouble) &&
      dstTy->isSpecificBuiltinType(BuiltinType::Float)) {
        llvm::Function *convFunc = CGF.CGM.getModule().getFunction("LD_to_posit32");
        if (!convFunc) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
              llvm::Type::getFP128Ty(CGF.getLLVMContext()),
              { llvm::Type::getDoubleTy(CGF.getLLVMContext()) }, false);
          convFunc = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                            "LD_to_posit32", &CGF.CGM.getModule());
        }
        return Builder.CreateCall(convFunc, Src, "ld_to_posit32");
      }
    }
  
    if (CE->getType()->isVectorType() && DestTy->isVectorType()) {
      // TODO: Support constrained FP intrinsics.
      QualType SrcElTy = CE->getType()->castAs<VectorType>()->getElementType();
      QualType DstElTy = DestTy->castAs<VectorType>()->getElementType();
      if (DstElTy->castAs<BuiltinType>()->getKind() <
          SrcElTy->castAs<BuiltinType>()->getKind())
        return Builder.CreateFPTrunc(Visit(CE), ConvertType(DestTy), "conv");
      return Builder.CreateFPExt(Visit(CE), ConvertType(DestTy), "conv");
    }
...
Value *ScalarExprEmitter::EmitDiv(const BinOpInfo &Ops) {
  ...
  if (CGF.getLangOpts().Fposit) {
    if(Ops.LHS->getType()->isDoubleTy()) {
      llvm::FunctionCallee PositDiv =
        CGF.CGM.getModule().getOrInsertFunction("posit64_2_div",
                                                Ops.LHS->getType(),
                                                Ops.LHS->getType(),
                                                Ops.RHS->getType());
    return CGF.Builder.CreateCall(PositDiv, {Ops.LHS, Ops.RHS}, "posit64_div_call");
    }else if(Ops.LHS->getType()->isFloatTy()) {
      llvm::FunctionCallee PositDiv =
        CGF.CGM.getModule().getOrInsertFunction("posit32_2_div",
                                                Ops.LHS->getType(),
                                                Ops.LHS->getType(),
                                                Ops.RHS->getType());
    return CGF.Builder.CreateCall(PositDiv, {Ops.LHS, Ops.RHS}, "posit32_div_call");
    }
  }
  if (Ops.Ty->isConstantMatrixType()) {
    llvm::MatrixBuilder MB(Builder);
...
Value *ScalarExprEmitter::EmitAdd(const BinOpInfo &op) {
  ...
  if (CGF.getLangOpts().Fposit){
    if(op.LHS->getType()->isDoubleTy()){
      llvm::FunctionCallee PositAdd =
      CGF.CGM.getModule().getOrInsertFunction("posit64_2_add",
                                              op.LHS->getType(),
                                              op.LHS->getType(),
                                              op.RHS->getType());
    return CGF.Builder.CreateCall(PositAdd, {op.LHS, op.RHS}, "posit_add_call");
    }else if(op.LHS->getType()->isFloatTy()){
      llvm::FunctionCallee PositAdd =
      CGF.CGM.getModule().getOrInsertFunction("posit32_2_add",
                                              op.LHS->getType(),
                                              op.LHS->getType(),
                                              op.RHS->getType());
    return CGF.Builder.CreateCall(PositAdd, {op.LHS, op.RHS}, "posit_add_call");
...
Value *ScalarExprEmitter::EmitSub(const BinOpInfo &op) {
  ...
  if (CGF.getLangOpts().Fposit){
    if(op.LHS->getType()->isDoubleTy()){
      llvm::FunctionCallee PositSub =
      CGF.CGM.getModule().getOrInsertFunction("posit64_2_sub",
                                              op.LHS->getType(),
                                              op.LHS->getType(),
                                              op.RHS->getType());
    return CGF.Builder.CreateCall(PositSub, {op.LHS, op.RHS}, "posit_sub_call");
    }else if(op.LHS->getType()->isFloatTy()){
      llvm::FunctionCallee PositSub =
      CGF.CGM.getModule().getOrInsertFunction("posit32_2_sub",
                                              op.LHS->getType(),
                                              op.LHS->getType(),
                                              op.RHS->getType());
    return CGF.Builder.CreateCall(PositSub, {op.LHS, op.RHS}, "posit_sub_call");
...
Value *ScalarExprEmitter::EmitCompare(const BinaryOperator *E,
  ...
  if (const MemberPointerType *MPT = LHSTy->getAs<MemberPointerType>()) {
    ...
    if (BOInfo.isFixedPointOp()) {
      ...
      if (CGF.getLangOpts().Fposit &&
          (BOInfo.LHS->getType()->isFloatTy() || BOInfo.LHS->getType()->isDoubleTy())) {

        llvm::Type *Ty = BOInfo.LHS->getType(); // unified type
        std::string CmpFnName;
        ...
        if (Ty->isFloatTy()) {
          ...
          llvm_unreachable("Unsupported FP type for posit");
        }

        switch (E->getOpcode()) {
          case BO_EQ: CmpFnName += "eq"; break;
          case BO_NE: CmpFnName += "ne"; break;
          case BO_LT: CmpFnName += "lt"; break;
          case BO_LE: CmpFnName += "le"; break;
          case BO_GT: CmpFnName += "gt"; break;
          case BO_GE: CmpFnName += "ge"; break;
          default:
            llvm_unreachable("Unsupported comparison op");
        }

        llvm::Function *CmpFn = CGF.CGM.getModule().getFunction(CmpFnName);
        if (!CmpFn) {
          llvm::FunctionType *FTy = llvm::FunctionType::get(
            llvm::Type::getInt1Ty(CGF.getLLVMContext()),
            { Ty, Ty }, false);
          CmpFn = llvm::Function::Create(FTy, llvm::Function::ExternalLinkage,
                                        CmpFnName, CGF.CGM.getModule());
        }

        Value *Result = Builder.CreateCall(CmpFn, {BOInfo.LHS, BOInfo.RHS}, "cmp.posit");
        return EmitScalarConversion(Result, CGF.getContext().BoolTy, E->getType(),
                                    E->getExprLoc());
      }
