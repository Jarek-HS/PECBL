...
//                             Expression Emission
//===--------------------------------------------------------------------===//
unsigned getScanfFormatArgIndex(StringRef FuncName) {
  if (FuncName == "scanf") return 0;
  if (FuncName == "fscanf") return 1; // fscanf(FILE*, fmt, ...)
  if (FuncName == "sscanf") return 1; // sscanf(char*, fmt, ...)
  return 0; // fallback
}
RValue CodeGenFunction::EmitCallExpr(const CallExpr *E,
  ...
  if (getLangOpts().Fposit) {
    if (const FunctionDecl *FD = dyn_cast_or_null<FunctionDecl>(E->getCalleeDecl())) {
      if (FD->getDeclName().getNameKind() == DeclarationName::CXXOperatorName &&
          FD->getOverloadedOperator() == OO_LessLess &&
          FD->getNumParams() == 1) {

        QualType ArgType = FD->getParamDecl(0)->getType();
        llvm::Value *Stream = EmitScalarExpr(E->getArg(0));
        llvm::Value *PositVal = EmitScalarExpr(E->getArg(1));

        llvm::Value *ConvResult = nullptr;
        llvm::Type *LDType = llvm::Type::getFP128Ty(getLLVMContext());
        std::string ConvFuncName;

        if (ArgType->isSpecificBuiltinType(BuiltinType::Float)) {
          ConvFuncName = "posit32_to_LD";
        } else if (ArgType->isSpecificBuiltinType(BuiltinType::Double)) {
          ConvFuncName = "posit64_to_LD";
        }

        if (!ConvFuncName.empty()) {
          llvm::Function *ConvFn = CGM.getModule().getFunction(ConvFuncName);
          if (!ConvFn) {
            llvm::Type *InputType = ArgType->isSpecificBuiltinType(BuiltinType::Float) ?
              llvm::Type::getFloatTy(getLLVMContext()) : llvm::Type::getDoubleTy(getLLVMContext());

            llvm::FunctionType *FT = llvm::FunctionType::get(LDType, {InputType}, false);
            ConvFn = llvm::Function::Create(FT, llvm::Function::ExternalLinkage, ConvFuncName, &CGM.getModule());
          }

          // inline the conversion into the operator call
          llvm::Value *LDouble = Builder.CreateCall(ConvFn, {PositVal});

          llvm::Function *OstreamLD = CGM.getModule().getFunction("_ZNSolsEe");
          if (!OstreamLD) {
            llvm::FunctionType *OFT = llvm::FunctionType::get(
              Stream->getType(), {Stream->getType(), LDType}, false);
            OstreamLD = llvm::Function::Create(OFT, llvm::Function::ExternalLinkage,
                                               "_ZNSolsEe", &CGM.getModule());
          }

          llvm::Value *Call = Builder.CreateCall(OstreamLD, {Stream, LDouble});
          return RValue::get(Call);
        }
      }
    }
  ...
  if (callee.isBuiltin()) {
    ...
    if((FuncName == "scanf"||FuncName=="fscanf") && getLangOpts().Fposit) {
      RValue RV = EmitBuiltinExpr(callee.getBuiltinDecl(), callee.getBuiltinID(),
                           E, ReturnValue);
      
      for (unsigned i = 0; i < E->getNumArgs(); ++i) {
        const Expr *Arg = E->getArg(i);
        QualType PtrTy = Arg->getType();
        QualType BaseTy = PtrTy->getPointeeType(); // 判斷 float / double
        if (BaseTy.isNull() || !BaseTy->isFloatingType())
          continue;
        llvm::Value *ArgPtr = EmitScalarExpr(Arg); // %5 / %6...
        if (BaseTy->isSpecificBuiltinType(BuiltinType::Float)) {
          llvm::Type *ElemTy = ConvertType(BaseTy);
          CharUnits Align = getContext().getTypeAlignInChars(BaseTy);
          Address Addr(ArgPtr, ElemTy, Align);
  
        // 做 load
          llvm::Value *Val = EmitLoadOfScalar(Addr, /*Volatile=*/false, BaseTy, E->getExprLoc());
  
          // 呼叫 float_to_posit32
          llvm::FunctionCallee ConvFn = CGM.getModule().getOrInsertFunction(
              "float_to_posit32",
              llvm::FunctionType::get(llvm::Type::getFloatTy(getLLVMContext()),
                                      {llvm::Type::getFloatTy(getLLVMContext())}, false));
          llvm::Value *PositFloat = Builder.CreateCall(ConvFn, Val);
  
          // 再 store 回去
          EmitStoreOfScalar(PositFloat, Addr, /*Volatile=*/false, BaseTy);

        } else if (BaseTy->isSpecificBuiltinType(BuiltinType::Double)) {
          llvm::Type *ElemTy = ConvertType(BaseTy);
          CharUnits Align = getContext().getTypeAlignInChars(BaseTy);
          Address Addr(ArgPtr, ElemTy, Align);
          ...
          llvm::FunctionCallee ConvFn = CGM.getModule().getOrInsertFunction(
              "double_to_posit64",
              llvm::FunctionType::get(llvm::Type::getDoubleTy(getLLVMContext()),
                                      {llvm::Type::getDoubleTy(getLLVMContext())}, false));
          llvm::Value *PositDouble = Builder.CreateCall(ConvFn, Val);
  
          EmitStoreOfScalar(PositDouble, Addr, /*Volatile=*/false, BaseTy);
        }
      }
      return RV;
    }
